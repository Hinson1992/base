...
public class RkQuery extends JDialog {

	private final JPanel contentPanel = new JPanel();
	private JTextField textField;
	private JTable table;
	private DefaultTableModel dftm;
	private JDateChooser jdc1,jdc2;
	...
	public RkQuery() {
		...
		comboBox.setModel(new DefaultComboBoxModel(new String[] {"商品名称", "商品编号", "供应商名称"}));
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				//将查询条件形参传入查询方法中
				vector=UserService.rkSearchInfo(conName, conOperation, content, vector,rdbtnNewRadioButton,sdate,edate);
				String[] tableHeads = new String[] { "入库编号", "商品编号", "商品名称", "规格", "单价",
						"数量", "金额", "供应商名称", "入库日期", "操作员", "经手人", "结算方式"  };
				//新建一个数据模型
		...
		button.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				TableModel model = new TableModel(tableHeads);
				//设置数据模型
				model.setDataRows(DaoFactory.getSellDao().rkSearch());
				//填充数据到table中
				table.setModel(model);
				//调用自动填充table方法
				Tools.fitTableColumns(table); 
