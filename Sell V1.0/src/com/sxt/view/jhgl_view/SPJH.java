...
public class SPJH extends JDialog {
	...
	public SPJH() {
		...
		final JLabel lblNewLabel_1 = new JLabel("进 货 编 号：");
		...
		JLabel label_2 = new JLabel("进货时间：");
		...
		this.addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				...
				long newId = DaoFactory.getSellDao().query(
						"select max(TRM_id) from ruku_main");
				newId = newId + 1;
				textField.setText(newId + "");

				msg = DaoFactory.getSellDao().gysSearch();
				Vector<String> khName = new Vector<String>();// 存放供应商名称
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < msg.size(); i++) {
					if (comboBox_1.getSelectedItem().equals(msg.get(i).get(1))) {
						TG_ID = msg.get(i).get(0).toString();
					}
				}
				String TRM_ID = textField.getText();
				String TRTM_PZS = data.size() + "";
				String TRTM_JE = allMoney + "";
				String TRTM_YSJL = textField_5.getText();
				java.sql.Date TRTM_DATE = new java.sql.Date(new java.util.Date().getTime());
				String TRTM_CZY = textField_7.getText();
				String TRTM_JSR = textField_3.getText();
				String TRTM_JSFS = comboBox.getSelectedItem().toString();
				Vector vector = new Vector<>();
				vector.add(TRM_ID);
				vector.add(TG_ID);
				vector.add(TRTM_PZS);
				vector.add(TRTM_JE);
				vector.add(TRTM_YSJL);
				vector.add(TRTM_DATE);
				vector.add(TRTM_CZY);
				vector.add(TRTM_JSR);
				vector.add(TRTM_JSFS);

				Boolean bool1 = DaoFactory.getSpDao().addJHD(vector);
				if (bool1) {
					JOptionPane.showMessageDialog(SPJH.this, "成功生成进货单,点击确定退出");
					count++;
				}
				...
				for (int i = 0; i < data.size(); i++) {
					...
					for (int j = 0; j < kcVector.size(); j++) {
						if (kcVector.get(j).contains(name)) {
							int sd = DaoFactory.getSellDao().sdQuery("select max(trd_id) from ruku_detail");
							sd += 1;
							String TRD_ID = sd + "";
							/*System.out.println(TRD_ID);*/
							String KC_ID = kcVector.get(j).get(0).toString();
							String TRM_DJ = kcVector.get(j).get(6).toString();
							String TRD_ID1 = TRD_ID;
							String TRD_SL = data.get(i).get(6).toString();
							/*System.out.println(SD_ID+KC_ID+SM_DJ+SM_ID1+SM_SL);*/
							Boolean bool = DaoFactory.getSellDao().addJHXD(
									TRD_ID, KC_ID, TRM_DJ, TRD_ID1, TRD_SL);
					...
					if (c > 0) {
						...
						JOptionPane.showMessageDialog(SPJH.this, "数据更新出错");
						break;
					}
				}
